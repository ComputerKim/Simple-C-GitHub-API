...
public static class GitHub {
    ...
    static readonly Regex defaultBranchRegex = new("default_branch\":\"([^\"]*)", RegexOptions.Compiled);
    ...
    public static string DefaultBranch(string owner, string repo) => Get($"https://api.github.com/repos/{owner}/{repo}", defaultBranchRegex).Select(x => x.Groups[1].Value).First();
    ...
    public static IEnumerable<(string path, string url)> Files(string owner, string repo) =>
        Files(owner, repo, DefaultBranch(owner, repo));
    ...
    public static byte[] Zip(string owner, string repo) => Zip(owner, repo, DefaultBranch(owner, repo));
